...
A Git commit is authoritative for the code at that commit. Jira is authoritative for a ticket's text, assignment, and workflow state. An imported document revision is authoritative for the bytes supplied by its author. BlueBridge records the extracted observation and is authoritative for the reviewed evidence versions, relationships, decisions, baselines, document snapshots, packages, and releases that BlueBridge controls.
...
flowchart LR
    ...
    Baseline --> Projection["Controlled projection<br/>document snapshot or package"]
...
| Controlled projection | What one approved template rendered from one exact baseline with one renderer version | New product truth, a replacement for the structured record, or proof that a package was submitted |
...
An import creates a stable source artifact and an immutable revision with a content hash. A PDF or DOCX import also stores the original bytes, SHA-256, extractor identity and version, warnings, and page- or paragraph-anchored text blocks. A later edit creates another revision. Generated candidates cite exact source text or an exact recorded clarification answer.
...
An approved template version and a named baseline create an immutable document snapshot. The snapshot stores its evidence-version membership, rendered model, renderer version, input fingerprint, and file hashes. A document package selects those snapshots and adds a manifest. The PDF, DOCX, or ZIP remains a projection of the structured record.
...
7. An author creates a planned release referencing that baseline and a fictional code revision.
8. WP-20A records manual fictional executions and QA decisions. WP-20B imports a downloaded CI bundle, records residual-risk decisions and simulated attestations, and permits a separate simulated release approver to approve the final state.
9. The local WP-30 workflow can render the baseline through an approved template and place the snapshot in a QA-reviewed package. That package does not change what the release proves.
...
This is a comparison of public product positioning, not an assessment of either product's internal architecture, validation evidence, or customer configuration. Official public pages were checked again on 18 September 2026.
...
| What is implemented here? | Fictional source, change, traceability, verification, residual-risk, final-release, and local document-package workflows. There are no live work-system connectors, authenticated identities, e-signatures, or production QMS claims. | Commercial product capabilities described on Infera's site. | Commercial product capabilities described on Ketryx's site. |
...
- The local PDF and DOCX workflow stores original bytes and extraction provenance, but it is uncommitted and unaccepted. It has no OCR, malware scan, qualified rendering process, or external document-system identity.
...
- Document packages contain baseline-derived outputs and a manifest. They are not submission packages and do not replace the structured record.
- The prototype has no electronic signature, retention, legal hold, permission administration, or validation package.

The prototype now demonstrates more of the philosophy than the earlier source-to-baseline slice did. Production authority still depends on authenticated identity, validated infrastructure, recoverable storage, real connectors, and approved procedures. Until those controls exist, BlueBridge is an evaluation workspace, not a quality-system claim.
