...
| WF-013 | The overview displayed an unexplained fixed score of 92. | The score was removed. The overview shows five deterministic findings, three fixtures, three high-severity findings, and no composite score. |
...
| WF-019 | New baselines retained relationships labelled as belonging to RR-1.0. | Approval now copies the curated relationship set into the new baseline with predecessor lineage. The overview and evidence browser resolve relationships only from the active baseline. |
